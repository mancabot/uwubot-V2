...
prefix = "!"
name = "𝙈𝙖𝙣𝙘𝙖-𝘽𝙤𝙩"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN 𝙈𝙖𝙣𝙘𝙖-𝘽𝙤𝙩😁"
rmenu = "HAI TEMEN 𝙈𝙖𝙣𝙘𝙖-𝘽𝙤𝙩👋 JANGAN LUPA SHOLAT 5 WAKTU YAA:)"
limitt = 10
ban = []
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const MancaCans = ["083815956151@s.whatsapp.net"] // ubah aja gapapa
			const manca = mek.message.conversation
		...
		case 'daftar':
					...
					if (isUser) return reply('kamu sudah Menjadi Temen 𝙈𝙖𝙣𝙘𝙖-𝘽𝙤𝙩:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
         ...
         case 'lirik':
                ...
                if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('62838159516151@s.whatsapp.net', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case 'map':
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat 𝙈𝙖𝙣𝙘𝙖-𝘽𝙤𝙩\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *𝙈𝘼𝙉𝘾𝘼-𝘽𝙊𝙏 BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[𝙈𝙖𝙣𝙘𝙖-𝘽𝙤𝙩]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
